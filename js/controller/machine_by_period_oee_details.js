...
$(document).ready(() => {
    ...
    checkIfMachineExist(machineId, loadData, [fromDate, toDate]);

});


function checkIfMachineExist(machineId, callback, [params]) {
    ...
    collection.doc(machineId).get().then((doc) => {
        if (!doc.exists) {
            ...
            callback(doc.data(), params[0], params[1]);
        }
    });
}

function loadData(machine, fromDate, toDate) {
